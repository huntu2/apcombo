...
int main (int argc, char * argv[])
{
  ...
  if(dump)printf("AT^SYSCONFIG=14,0,3FFFFFFF,1,2\n");
    serial_port_write("AT^SYSCONFIG=14,0,3FFFFFFF,1,2\r\n");
    len=getser(data,timeout);
    if(len){
        if(dump)fwrite(data,1,len,stdout);
        pb=(char *) buffer;
    ...
    if(dump)printf("AT^SYSCONFIG?\n");
    serial_port_write("AT^SYSCONFIG?\r\n");
    len=getser(data,timeout);
    if(len){
        if(dump)fwrite(data,1,len,stdout);
        pb=(char *) buffer;
